...
## Git vs GitHub?
A common misconception i've heard peers have is that Git and GitHub are the same thing! NOPE Infact their functions are completely unique to one another!
...
Here are basic linux commands that are good to know: https://www.guru99.com/linux-commands-cheat-sheet.html
...
Now we will go back to the Git Bash and configure our Github user on there. Lets first cd (change directory) into the directory we want to use

Then lets run the following commands: git config --global user.email yourgithubemail@blank.edu git config --global user.name "Your Github Username" git config –global user.password “your ssh key” git config --global user.password "1234321" For the configuration of the password above, add in your ssh key.

To save your configuration!!! Otherwise you’ll have to resign in every time you try to access Github using git. git config --global credential.helper store

To see your newly saved credentials, be aware that this will show your password as a string! git config --list

Okay so - if all has gone well you can now..

##Clone a repository You can now go and clone my repository for this project!

Go to the following link: https://github.com/Alinamoo/IntroToGit
...
Copy the SSH link from the newly created repository like we did when cloning the first repository: git remote add origin git@github.com:yourusername/yourreponame.git git push -u origin main
...
Stage and Share
git status You'll see all the files you've made edits to. The files in red are those that have edits and can be added your commit.
